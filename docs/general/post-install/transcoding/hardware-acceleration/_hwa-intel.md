...
This tutorial guides you on setting up full video hardware acceleration on Intel integrated GPUs and ARC discrete GPUs via QSV and VA-API. If you are on macOS, please use [VideoToolbox](./?method=apple) instead.
...
You only need to follow the [Windows Setups](./?method=intel#windows-setups) and [Linux Setups](./?method=intel#linux-setups) to configure and verify it.
...
4. Check to the remaining parts of [Debian And Ubuntu Linux](./?method=intel#debian-and-ubuntu-linux).
