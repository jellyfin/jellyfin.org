...
Keep in mind that the `--label "io.containers.autoupdate=image"` flag will allow the container to be automatically updated via `podman auto-update`. If using this option, remember that a [backup](/docs/general/administration/backup-and-restore) will be required to restore a previous version.
