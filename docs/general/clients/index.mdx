...
Clients that meet one or more of the following criteria may be considered for removal from the list:

- The client is no longer maintained
- The client is broken
- The client no longer supports a modern version of Jellyfin

In order for a client to be removed, the following has to happen:

1. The Jellyfin team must attempt to reach out to the original author or a maintainer designated by the author, and should wait for at least one month for a reply.
2. If the author or a maintainer cannot be reached after one month, the client shall be removed.
3. If the links added to the Jellyfin clients page are actively being used for spam and/or malware, the client may be removed without reaching out first.
