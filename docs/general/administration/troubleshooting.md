...
Due to a quirk in how Jellyfin currently works, media items in the Jellyfin databae may not reflect the current filesystem, which is working as-intended to preserve user data, but as it never expires and ends being purged, can cause problems with your Jellyfin server.
...
If the directory is empty, you will no longer have user data relating to positions in a piece of media, history of playback et al.
