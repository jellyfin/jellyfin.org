...
1. We have [a new repository browser UI](https://repo.jellyfin.org) with a new file layout, on a new master repository machine, built by new CI, that will hopefully make it nicer to look around and get right to what you need. This is still a bit of a work in progress, so please report any bugs you find to us! Note that most old links will still work but some *may* be broken; please check the new page for updated links.
