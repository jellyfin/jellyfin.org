...
was also hard to work with making it challenging to fix the issues or implement new features. To encounter this we decided to rewrite all
...
planned features include showing lyrics, enabling SyncPlay and support gapless playback.

Work has already started to also enable video playback based on this new code so we can remove the old and add all kinds of new features
with ease.
...
a different method for getting the results. With these changes a search on large servers will be noticeably faster and no flickering occurs
anymore.
...
Update your app now to experience all these changes! The app stores will auto-update your Jellyfin app if you're already using the app. For
