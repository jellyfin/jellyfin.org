...
The migration will aggregate the old `library.db` into the `jellyfin.db` file and then rename it to `library.db` so the unstable builds will no longer be compatiable with previous versions.
