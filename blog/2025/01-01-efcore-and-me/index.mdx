...
[Josh’s blog post](../../2024/11-03-efcore-refactoring-incoming/index.mdx) in November 2024 outlined much of what we aimed to achieve, and I want to take this opportunity to address the feedback and questions we've received, clarifying a few key points.  

The old code contained a lot of code that was directly accessing the underlying SQLite database. 
...
There are also already plans that build upon the new architecture to provide proper means to backup a Jellyfin instance now everything is strongly typed. This could mean a plugin that can backup the whole Jellyfin instance, while it is running which is currently not possible to do reliably.
Further would this export/import plugin be needed if we want to support multiple database providers to export data to other databases.
