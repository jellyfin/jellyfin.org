...
SQ-Lite has a feature that tries to get around this limitation, the Write-Ahead-Log is an separate file that acts as an journal of operations that should be applied to an SQ-Lite file so within an application multiple parallel writes might take place and get enqueued into the WAL and when another part wants to read data it reads from the actual database and then scans the WAL for modifications and applies them on the fly.
...
This seems to be a not uncommon issue there are many reports to be found researching the issue.

The factor that makes this issue so bad is that it does not happen reliably. So far we only have one Team member where this can be (somewhat) reliably be reproduced makes this even worse a bug. From the reports this issue happens across all Operating systems, HDD speeds and with or without virtualization. So we do not have any deciding factor identified that even contributes to the likeliness of the issue happening.
...
The overall idea is to have multiple strategies of locking, because all levels of synchronization will inevitable at the cost of performance we only want to do it when its really necessary.
